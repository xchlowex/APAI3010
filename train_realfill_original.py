...
class DINOv2GeometricAdapter(nn.Module):
    def __init__(self, dinov2_model, sd_dim=768):
        super().__init__()
        # Store the model passed from outside
        self.dinov2 = dinov2_model 
        
        # Now this works because self.dinov2 exists!
        for param in self.dinov2.parameters():
            param.requires_grad = False  # Freeze DINOv2
 
        # 2. Get the input dimension dynamically (384 for ViT-S)
        dinov2_out_dim = self.dinov2.embed_dim
            
        # Projection layer to map DINOv2 (384 dim) to SD (768 dim)
        self.proj = nn.Sequential(
            nn.Linear(dinov2_out_dim, sd_dim),
            nn.LayerNorm(sd_dim),
            nn.GELU(),
    ...
    def forward(self, ref_image):
        # ref_image should be normalized to DINOv2 standards
        # patch_tokens= 16 x 16 = 256 total patches
        # dtype = self.dinov2.patch_embed.proj.weight.dtype
        dtype = next(self.dinov2.parameters()).dtype
        
        with torch.no_grad():
            features = self.dinov2.forward_features(ref_image.to(dtype))            # features['patch_tokens'] contains the geometric structure        
            # patch_tokens = features["patch_tokens"] # Shape: [Batch, 1024, 384]
            patch_tokens = features["x_norm_patchtokens"]
            
        geometric_embeddings = self.proj(patch_tokens)
        return geometric_embeddings # [Batch, 1024, 768]


def frequency_weighted_loss(pred_noise, target_noise, alpha_hf=2.0):
    
    """
    Calculates MSE loss between frequency components of the predicted and target noise.
...
class RealFillDataset(Dataset):
    ...
    def __init__(
        ...
        #     os.path.join(self.train_images_dir, f, self.target_image) 
        #     for f in sorted(os.listdir(self.ref_data_root.iterdir())) 
        #     if f.lower().endswith(valid_extensions)
        # ]
        # # if hasattr(self, 'target_image') and self.target_image not in self.train_images_path:
        # #     self.train_images_path.append(self.target_image)
    ...
    # 6. FIXED: Select and transform the DINO reference image
        ...
        ref_idx = random.randint(0, len(self.train_images_path) - 2)
...
def main(args):
    ...
    # SWITCHED TO HERE
    # ==========================================
    # NEW CONSOLIDATED SETUP BLOCK STARTS HERE
    # Notice the indentation is back to the main level!
    # ==========================================

    # 1. DEFINE WEIGHT DTYPE FIRST
    ...
    # Move vae to device and cast to weight_dtype here
    vae.to(accelerator.device, dtype=weight_dtype)

    # 2. INITIALIZE THE ADAPTER
    adapter = DINOv2GeometricAdapter(dinov2_model, sd_dim=unet.config.cross_attention_dim)
    adapter.dinov2.to(accelerator.device) 
    adapter.to(accelerator.device)

    # 3. CREATE ONE UNIFIED OPTIMIZER
    optimizer = optimizer_class(
        [
            {"params": unet.parameters(), "lr": args.unet_learning_rate},
            {"params": text_encoder.parameters(), "lr": args.text_encoder_learning_rate},
            {"params": adapter.proj.parameters(), "lr": args.unet_learning_rate} # Adapter included!
        ],
    ...
    # 6. ONE UNIFIED ACCELERATOR PREPARE
    ...
    # # 2. INITIALIZE THE ADAPTER
    # adapter = DINOv2GeometricAdapter(sd_dim=unet.config.cross_attention_dim)
    # adapter.dinov2.to(accelerator.device, dtype=weight_dtype) # Ensure backbone matches
    ...
    # # 6. ONE UNIFIED ACCELERATOR PREPARE
    # # Everything is defined, so accelerator can wrap them all at once
    # unet, text_encoder, adapter, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
    #     unet, text_encoder, adapter, optimizer, train_dataloader, lr_scheduler
    # )
...
# OLD Dataset and DataLoaders creation:
    # train_dataset = RealFillDataset(
    #     train_data_root=args.train_data_dir,
    #     tokenizer=tokenizer,
    #     size=args.resolution,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, text_encoder, adapter):
                ...
                geom_hidden_states = adapter(batch["ref_pixel_values"].to(dtype=weight_dtype))
                ...
                # 3. Predict Noise
                model_pred = unet(
                    inputs, # MUST use 9-channel inputs
                    timesteps, 
                ...
                loss = frequency_weighted_loss(model_pred, noise, alpha_hf=2.0)
            ...
            ####################################
                            ...
                            #     with accelerator.accumulate(unet, text_encoder):
                            #         # ... existing latent/conditioning code ...
                                
                            #         # 1. Get text embeddings
                            #         encoder_hidden_states = text_encoder(batch["input_ids"])[0]
                                
                            #         # 2. Extract Geometric Features
                            #         # batch["ref_pixel_values"] comes from your updated Dataset
                            #         geom_hidden_states = adapter(batch["ref_pixel_values"].to(dtype=weight_dtype))
                                
                            #         # 3. CONCATENATE: This is the core "blueprint" injection
                            #         # We append the geometric tokens to the text tokens
                            #         combined_hidden_states = torch.cat([encoder_hidden_states, geom_hidden_states], dim=1)
                            ...
                            #             noisy_latents, 
                            #             timesteps, 
                            #             combined_conditioning, # Use the combined states here!
                            #             added_cond_kwargs=added_cond_kwargs # if using SDXL
                            #         ).sample
                            #         ############################
    ...
    if accelerator.is_main_process:
        pipeline = StableDiffusionInpaintPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
            unet=accelerator.unwrap_model(unet.merge_and_unload(),
